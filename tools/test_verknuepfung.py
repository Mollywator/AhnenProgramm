...
def kleine_familie() -> dict:
    """Ida married Chris, they have a child; Chris has a father of his own."""
    return {"meta": {}, "people": [
        person(1, "Ida",        spouses=[2], children=[3]),
        person(2, "Chris",      spouses=[1], children=[3], parents=[4]),
        person(3, "Kind",       parents=[1, 2]),
        person(4, "Chris Vater", children=[2]),
    ], "marriages": []}
...
class TestKnuepfen(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ergebnis = v.knuepfen(
            self.quelle, "rodenberg", 1, self.ziel, "loeschen",
    ...
    def test_die_verknuepfte_person_kommt_an(self):
        self.assertIn("Ida", self.dort)

    def test_die_angehakten_gruppen_kommen_mit(self):
        self.assertIn("Chris", self.dort)      # Ehepartner
        self.assertIn("Kind", self.dort)       # Kinder

    def test_was_nicht_angehakt_war_bleibt_zurueck(self):
        # "Eltern" was not ticked, so Chris' father stays in his own tree.
        self.assertNotIn("Chris Vater", self.dort)

    def test_sie_selbst_ist_nicht_geliehen(self):
        # She is in the second tree in her own right - which is what puts her
        # into its "Alle" view.
        self.assertFalse(v.geliehen(self.dort["Ida"]))
    ...
    def test_die_mitgereisten_sind_geliehen(self):
        ...
        self.assertTrue(v.geliehen(self.dort["Chris"]))
        self.assertTrue(v.geliehen(self.dort["Kind"]))
        self.assertEqual(v.link_von(self.dort["Chris"])["via"],
                         self.ergebnis["uid"])

    def test_beide_seiten_kennen_einander(self):
        hier = self.quelle["people"][0]
        self.assertEqual(v.uid_von(hier), v.uid_von(self.dort["Ida"]))
        self.assertEqual(v.andere_baeume(hier, "rodenberg"),
                         [{"slug": "loeschen", "id": int(self.dort["Ida"]["id"])}])
    ...
    def test_nummern_sind_ortsgebunden(self):
        ...
        self.assertGreaterEqual(int(self.dort["Ida"]["id"]),
                                person_lib.FIRST_NEW_ID)

    def test_verbindungen_werden_auf_die_neuen_nummern_umgeschrieben(self):
        ida, chris, kind = self.dort["Ida"], self.dort["Chris"], self.dort["Kind"]
        self.assertEqual(ida["spouses"], [int(chris["id"])])
        self.assertEqual(sorted(kind["parents"]),
                         sorted([int(ida["id"]), int(chris["id"])]))

    def test_kein_draht_ins_nichts(self):
        # Chris' father did not travel, so Chris must not point at him over
        # there - his number means somebody else in that file.
        self.assertEqual(self.dort["Chris"]["parents"], [])

    def test_zweimal_verknuepfen_verdoppelt_niemanden(self):
        v.knuepfen(self.quelle, "rodenberg", 1, self.ziel, "loeschen",
                   ["spouses", "children"], person_lib.blank_person, naechste_id)
        namen = [p["name"] for p in self.ziel["people"]]
        self.assertEqual(sorted(namen), ["Chris", "Ida", "Kind"])
...
class TestNameIstKeinBeweis(unittest.TestCase):
    ...
    def test_gleiche_namen_werden_gemeldet_nicht_verschmolzen(self):
        ziel = {"meta": {}, "people": [
            person(1001, "Ida", birth={"year": 1990, "month": None, "day": None,
                                       "place": None, "approx": False}),
        ], "marriages": []}
        gefunden = v.schon_drueben(ziel, ["Ida"])
    ...
    def test_ein_namensgleicher_wird_nicht_wiederverwendet(self):
        ...
        ziel = {"meta": {}, "people": [person(1001, "Ida")], "marriages": []}
        v.knuepfen(quelle, "rodenberg", 1, ziel, "loeschen", [],
                   person_lib.blank_person, naechste_id)
        # The Ida who was already there keeps to herself; the linked one is new.
...
class TestGeteiltUndOertlich(unittest.TestCase):
    ...
    def test_person_reist_baum_bleibt(self):
        geteilt = v.einsammeln(person(1, "Ida", occupation="Hebamme",
                                      spouses=[2], photo="ida.jpg"))
...
class TestZusammenfuehren(unittest.TestCase):
    ...
    def test_listen_werden_vereinigt_ohne_rueckfrage(self):
        hier = person(1, "Ida", events=[{"kind": "Heirat", "year": 2015}])
        dort = person(9, "Ida", events=[{"kind": "Beruf", "year": 2012}])
    ...
    def test_eine_zweite_heirat_ist_kein_widerspruch(self):
        hier = person(1, "Ida", events=[{"kind": "Heirat", "year": 2015}])
        dort = person(9, "Ida", events=[{"kind": "Heirat", "year": 2021}])
        vereint, streit = v.zusammenfuehren(hier, dort)
        self.assertEqual(len(vereint["events"]), 2)
        self.assertEqual(streit, [])
    ...
    def test_doppelte_eintraege_werden_nicht_verdoppelt(self):
        ...
        vereint, _ = v.zusammenfuehren(person(1, "Ida", events=[eintrag]),
                                       person(9, "Ida", events=[eintrag]))
    ...
    def test_zwei_geburtsdaten_koennen_nicht_beide_gelten(self):
        ...
        _, streit = v.zusammenfuehren(person(1, "Ida", birth=a),
                                      person(9, "Ida", birth=b))
        self.assertIn("birth", streit)

    def test_gleiche_werte_sind_kein_streit(self):
        _, streit = v.zusammenfuehren(person(1, "Ida", occupation="Hebamme"),
                                      person(9, "Ida", occupation="Hebamme"))
        self.assertEqual(streit, [])

    def test_ein_leeres_feld_nimmt_was_die_andere_seite_weiss(self):
        vereint, streit = v.zusammenfuehren(person(1, "Ida"),
                                            person(9, "Ida", occupation="Hebamme"))
...
class TestSpiegeln(unittest.TestCase):
    ...
    def _zwei_baeume(self):
        rot = self.store.empty_tree("Rodenberg")
        rot["people"] = kleine_familie()["people"]
        rot_slug = self.store.create("Rodenberg", rot, open_it=True)
        gelb_slug = self.store.create("Loeschen", self.store.empty_tree("Loeschen"),
                                      open_it=False)
        quelle = self.store.load(rot_slug)
        ziel = self.store.load(gelb_slug)
        v.knuepfen(quelle, rot_slug, 1, ziel, gelb_slug, ["spouses"],
                   person_lib.blank_person, self.store.next_id)
        self.store.save(ziel, gelb_slug)
        self.store.save(quelle, rot_slug)
        return rot_slug, gelb_slug

    def test_eine_aenderung_drueben_steht_auch_hier(self):
        rot_slug, gelb_slug = self._zwei_baeume()
        gelb = self.store.load(gelb_slug)
        ida = next(p for p in gelb["people"] if p["name"] == "Ida")
        ida["occupation"] = "Hebamme"
        self.store.save(gelb, gelb_slug)

        rot = self.store.load(rot_slug)
        hier = next(p for p in rot["people"] if int(p["id"]) == 1)
        self.assertEqual(hier["occupation"], "Hebamme")

    def test_die_oertlichen_felder_bleiben_unberuehrt(self):
        rot_slug, gelb_slug = self._zwei_baeume()
        gelb = self.store.load(gelb_slug)
        ida_dort = next(p for p in gelb["people"] if p["name"] == "Ida")
        ida_dort["occupation"] = "Hebamme"
        self.store.save(gelb, gelb_slug)
    ...
    def test_ein_baum_weniger_nimmt_die_person_nicht_mit(self):
        rot_slug, gelb_slug = self._zwei_baeume()
        self.store.discard(gelb_slug)

        rot = self.store.load(rot_slug)
        ida = next((p for p in rot["people"] if int(p["id"]) == 1), None)
        self.assertIsNotNone(ida, "die Person darf mit dem Baum nicht verschwinden")
        self.assertEqual(ida["name"], "Ida")
    ...
    def test_ein_fehlender_baum_wird_gemeldet_und_kostet_nicht_das_speichern(self):
        ...
        rot = self.store.load(rot_slug)
        ida = next(p for p in rot["people"] if int(p["id"]) == 1)
        ida["occupation"] = "Hebamme"
        self.store.save(rot, rot_slug)

        self.assertTrue(self.store.LETZTE_SPIEGEL_BERICHTE,
                        "der fehlende Baum muss gemeldet werden")
        wieder = self.store.load(rot_slug)
    ...
    def test_jeder_baum_bleibt_fuer_sich_lesbar(self):
        _, gelb_slug = self._zwei_baeume()
        with open(self.store.tree_path(gelb_slug), encoding="utf-8") as fh:
            allein = json.load(fh)
        ida = next(p for p in allein["people"] if p["name"] == "Ida")
